...
class Screen(QThread):  # 逐帧检测
        ...
        while True:
            ...
            if framing.empty() is not True:
                ...
                for x1, y1, x2, y2, text in self.mylabel.bboxPointList:
                    ...
                    if is_black:
                        duration = self.record_black_screen_duration(self.time)
                        if duration > self.specific_duration:
                            power_status = "长时间黑屏"
    ...
    def record_black_screen_duration(self, time_value):
        if self.start_time is None:
            self.start_time = time_value
        else:
            self.end_time = time_value
            self.labels.append((self.start_time, self.start_time))  # 将开始和结束时间添加到标签列表中
            duration = self._time2second()
            print("黑屏持续时间为：", duration) #或者将 duration 存储到数据库中等等
            # 重置变量，以便记录下一次黑屏时间
            self.start_time = None
            self.end_time = None
